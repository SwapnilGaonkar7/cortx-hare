...
class CdfGenerator:
    ...
    def _create_pool_descriptions(self) -> List[PoolDesc]:
        pools: List[PoolDesc] = []
        conf = self.provider
        cluster_id = conf.get('cluster>cluster_id')
        ...
        for x in range(storage_set_count):
            ...
            #TODO We also need to add 'spare units' in below check
            if not data_devices_count >= data_units_count + parity_units_count:
